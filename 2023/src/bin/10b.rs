...
fn main() {
    ...
    let (start_pos, map) = parse(input.clone());
    ...
    let mut filtered_map: HashMap<Pos, Tile> = HashMap::new();
    filtered_map.insert(start_pos, Tile::S);
    ...
    for y in 0..h {
        ...
        for x in 0..w {
            if let Some(&t) = filtered_map.get(&Pos(x, y)) {
                if t == Tile::NS || t == Tile::NE || t == Tile::NW || t == Tile::S {
                    inside = !inside;
                }
            } else if inside {
                area += 1
